...
\begin{lemma}
    ...
    Suppose \(J\subseteq J'\) are both closed and connected, and \((J'\setminus J)\cap  \mathbb{Z}  = \{m\}\). Then \(Z_{J'}\) is homotopy equivalent to the union of \(Z_J\) and a cone of \(lk_\downarrow(x,\tilde X_L)\) for each \(x\in \tilde f^{-1}(m) \cap \tilde X_L^{(0)}\).
\end{lemma}
\begin{proof}
...
TODO L decides \(K_L\) properties. D2: WTP if L is connected but not simply connected, then \(K_L\) is finitely presented. ... did we give up this route?

TODO end of D2: In fact, any \(Z_J\) is homotopy equivalent to a wedge of copies of \LL(L\), one for each vertex outside \(Z_J\).

D3: recall: if L is not s.conn, then \(K_L\) is not fp