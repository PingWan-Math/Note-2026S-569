...
The case at \(2\) is still an open question, though there is already a potential counterexample. 
...
\begin{theorem}
    \label{thm:type-FF-implies-FP}
    Type FF implies type FP.
\end{theorem}
...
Right-angled Artin groups is a class of groups that can tells those properties apart.
...
\begin{theorem}
    ...
    Let \(k\) be the clique number (i.e. maximal size of complete subgraph) of \(\G\). Then \(\gd{A_G}=k\).
\end{theorem}
