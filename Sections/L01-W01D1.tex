...
\begin{definition}[\(K(G,1)\)]
    \label{def:KG1}
    Let \(G\) be a group.
    A \emph{\( K(G,1) \) }is a connected space \(X\) such that \(\pi_1(X) \cong G\)  and  whose universal covering space \(\tilde X\) is contractible.
...
For \(n\geq 2\), any map \((\mb{S}^n , \star) \to (X,x_0)\) lifts to \((\mb{S}^n , \star) \to (\tilde X, \tilde x_0)\) (Since \(\pi_1(\mb{S}^n ,\star)\) is trivial). If \(X\) is aspherical, then \((\mb{S}^n , \star) \to (\tilde X, \tilde x_0)\) is null-homotopic. Hence \(\pi_n(X,x_0) =0\). This proves one direction of \cref{thm:aspherical-criterion-trivial-homotopy-group}. The other direction is harder, and we leave it for future discussion.                   
...
We introduce a related space property below.
...
\begin{example}[Non-aspherical Spaces]
    Connect sums of aspherical spaces typically produce non-aspherical spaces.
\end{example}
...
\begin{restatable}{theorem}{TorsionCriterion}
    \label{thm:torsion-criterion-KG1}
    If \(G\) has non-trivial torsion, then there does not exist any finite-dimensional \(K(G,1)\). 
\end{restatable}
Proof in \cref{proof:torsion-criterion-KG1}.
Another way to phrase \cref{thm:torsion-criterion-KG1} is: if \(G\) has a finite-dimensional \(K(G,1)\), then it's torsion free.
...
\begin{restatable}{definition}{TypeF}
...
\begin{restatable}{definition}{GeomDim}
    \label{def:geometric-dimension}
      A group \(G\) has \emph{finite geometric dimension} if it has a finite-dimensional \(K(G,1)\). The \emph{geometric dimension}, denoted as \(\gd{G}\), is the minimal dimension of all of its \(K(G,1)\).     
\end{restatable}

It is sometimes difficult to understand the \(K(G,1)\) of a group. We thus use multiple invariants to help determine those types. If \(G\) is a group and \(X\) is a \(K(G,1)\), then the algebro-topological invariants of \(X\) are invariants of \(G\). In particular, we look at the cohomology of groups (with trivial coefficients. See \cref{def:cohomology-of-group}) :
\[ H^*(G,\Z):= H^*(X,\Z) \]