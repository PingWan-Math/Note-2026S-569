...
It is sometimes difficult to understand the \(K(G,1)\) of a group. We thus use multiple invariants to help determine those types. If \(G\) is a group and \(X\) is a \(K(G,1)\), then the algebro-topological invariants of \(X\) are invariants of \(G\). In particular, we look at the cohomology of groups (with trivial coefficients):
\[ H^*(G,\Z):= H^*(X,\Z) \]