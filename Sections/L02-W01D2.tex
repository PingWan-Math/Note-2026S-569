...
Notice that if  \(G\) has a non-trivial torsion, that is, \(g^n  =1\) for some non-trivial element \(g\in G\),  then we have: 
...
A motivation for \cref{conj:torsion-free-no-zero-divisor} is that we may hope to embed \(\Z G\) into a division ring, so we wish to have no non-zero zero-divisors in \(\Z G\).
...
\begin{definition}[Free resolution]
    ...
    A free resolution of \(\Z\) (the trivial \(\Z G\)-module) by free \(\Z G\)-modules is a resolution
    \[ \cdots\to F_1 \to F_0 \xrightarrow{\epsilon} \Z \to 0\]
    that is exact, and every \(F_i\) is a free \(\Z G\)-module (i.e. \(F_i = (\Z G)^n\) for some \(n\)).
\end{definition}


Sometimes we also use resolutions by projective \(\Z G\)-modules (See \cref{def:projective-module}).

\begin{remark}
    \label{rmk:free-resolution-arises-naturally}
    A free resolution arises naturally from a \(K(G,1)\) in the following manner. Say \(X\) is a CW complex \(K(G,1)\) (recall \cref{def:KG1}). Then any \(i\)-cell in \(\tilde X\) is \(g\sigma\) for some \(g\in G\) and  some \(i\)-cell \(\sigma \) in \(X\). So we can view the cellular \(i\)-chain \(C_i(\tilde X; \Z)\) as a free \(\Z G\)-module whose basis is in bijection with \(i\)-cells in \(X\). 
...
\begin{theorem}
    ...
    There is a unique resolution of \(\Z\) by free (projective) \(\Z G\)-module up to homotopy.
\end{theorem}


\begin{definition}[Cohomology of group]
    \label{def:cohomology-of-group}
    Let \(M\) be a left \(\Z G\)-module. Let \(F = \{(F_i, d_i)\}\) be a free resolution of \(\Z\). We truncate \(F\)  to get a complex (that is non-exact at $F_0$):
    ...
    Here \(d^{i} : \Hom{F_i}{M}{\Z G} \to \Hom{F_{i+1}}{M}{\Z G} \) is defined by \(d^i(f) = f\circ d_{i+1}\).
...
\]
Let  \(M^G = \{m\in M \mid \forall g \in G, gm=m\}\). Notice that \(H^0(G;M)= \ker d^0 = \im \epsilon^* \cong \Hom{\Z }{M }{\Z G}\). Recall that \(G\) acts on \(\Z\) trivially, so elements in \(\Hom{\Z }{M }{\Z G}\) have to be elements in \(M\) that are invariant under \(G\), i.e. \(H^0(G;M) = M^G\).

Since free resolutions  are chain homotopic, \(H^*(G;M)\) is independent of the choices of the free resolution. 

\begin{example}[Cohomology of \(\Z/n\Z\)]
    \label{ex:cohomology-of-ZnZ}
    Let \(G=\Z/n\Z\), \(M=\Z\). Then \(\Z G = \Z[t]/(t^n-1)\). Consider \(\Z G \xrightarrow{\epsilon} \Z \to 0\). Here \(\ker \epsilon = (t-1)\). Let \(N = t^{n-1}+\dots + t + 1\). We have the free resolution:
    \[ 
        \cdots \to \Z G
...
Apply \(\Hom{\_ }{\Z }{\Z G}\)to the resolution. Since \(G\) acts on \(\Z\) trivially,  any  \(f\in \Hom{\Z G }{\Z }{\Z G}\) has to map all \(g \in G\) to a constant. Therefore, \(\Hom{\Z G }{\Z }{\Z G} \cong \Z\). Thus the cochain complex we obtained is:
\[
...
\begin{proposition}
    ...
    If \(X\) is an \(n\)-dimensional \(K(G,1)\) and \(M\) is any \(\Z G\)-module, then \(\forall m > n\), \(H^m(G;M) =0\).
\end{proposition}
\begin{proof}
    The free resolution arising from \(X\) (See \cref{rmk:free-resolution-arises-naturally}) is trivial for \(m>n\).
\end{proof}
...
\begin{proof}
\label{proof:torsion-criterion-KG1}
    ...
    If \(G\) has non-trivial torsion, then there exists \(\Z/n\Z \leq G\) with \(n\geq 2\). Let \(H = \Z /n \Z\). By \cref{cor:KG1-ZnZ-infinite-dimensional}, \(X^H\) is infinite-dimensional, hence so is \(X\).
\end{proof}