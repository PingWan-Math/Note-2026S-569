...
\]
Denote \(H^0(G;M)\) as \(M^G\). Notice that \(M^G = \ker d^0 = \im \epsilon^* \cong \Hom{\Z }{M }{\Z G}\). Recall that \(G\) acts on \(\Z\) trivially, so elements in \(\Hom{\Z }{M }{\Z G}\) have to be elements in \(M\) that are invariant under \(G\), i.e. \(M^G = \{m\in M \mid \forall g \in G, gm=m\}\).
...
\end{proposition}
\begin{proof}
    The free resolution is trivial for \(m>n\).
\end{proof}
