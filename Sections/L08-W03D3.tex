...
\begin{example}[Examples of cube complex]
    ...
    \begin{itemize}
        \item \(\mathbb{S}^1\) and any \(1\)-dimensional CW complex.
        \item Tori.
        \item \(\mathbb{S}^2\)
        ...
        \item Seifert–Weber dodecahedral space.
    \end{itemize}
\end{example}
...
Note that the link of the only vertex in \(X_L\) may not be \(L\).
\begin{proof}
    A \(k\)-simplex in \(L\) gives a \(k+1\)-cube(torus) in \(X_L\). The link then is joint of \(k+1\) \(\mathbb{S}^0\)'s, which is simplicial.

    TODO   Exercise: if \(L\) is flag then the link is also flag.
\end{proof}
...
\begin{definition}
    ...
    TODO a formalized way to define hyperplane.
\end{definition}
