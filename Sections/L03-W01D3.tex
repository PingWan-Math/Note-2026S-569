...
\begin{lemma}[{\cite[Lemma VIII.7.4]{brown_cohomology_1982}}]
    ...
    Let \(M\) be a \(\Z G\)-module, then 
    \[
        \Hom{M }{\Z G }{\Z G} \cong \Hom{M }{\Z }{c}.
    \]
\end{lemma}
\begin{proof}
    Take \(F \in \Hom{M }{\Z G }{\Z}\). For \(m\in M\), \(F(m)\) can be written as \(\sum_{g\in G} f_g(m) g\) where \(f_g(m)=0\) for all but finitely many \(g\).
...
\begin{remark}
    The requirement of local compactness is to ensure that \(\d_c^i(C_c^i(Y;\Z)) \subseteq C_c^{i+1}(Y;\Z)\).
\end{remark}
...
\begin{example}[Compactly supported cohomology of \(\R\)]
    ...
    Since \(\R\) is non-compact, \(H^0_c(\R;\Z)=0\).  For \(n >2\), since there are no \(n\)-cells, \(H^n_c(\R;\Z)=0\). Consider now \(H^1_c(\R;\Z)\). Since there are no \(2\)-cells, all \(1\)-cochains are \(1\)-cocycles. Therefore \(H^1_c(\R;\Z)=C_c^1(\R;\Z)/\im \d^0_c\).
